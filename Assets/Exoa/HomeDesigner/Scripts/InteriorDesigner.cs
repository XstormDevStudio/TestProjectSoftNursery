...
namespace Exoa.Designer
{
    public class InteriorDesigner : MonoBehaviour
    {
        ...
        public GameObject CreateObj(SceneObject so, bool isSceneObject, bool applyScaleMultiplier, Transform parent)
        {
            ...
            lastObject.transform.localScale = so.scale == Vector3.zero ? lastObject.transform.localScale : scaler;
            ...
            return lastObject;
            scaler = Vector3.one;
            ghostObject.transform.Rotate( new Vector3(0, 0, 0));
        }
        ...
        private void Update()
        {
            ...
            if (!overUI && currentPrefabOptions != null)
            {
                ...
                if (hits.Length > 0)
                {
                    ...
                    if (!isRotating && !isTap && !escapePressed)
                    {
                        ...
                        ghostObject.transform.rotation = Quaternion.Euler(0, 5 * turns, 0) * ghostObject.GetComponent<ModuleController>().GetInitRotation();
                    ...
                    else
                    {
                        ...
                        if (!positionTaken && !altPressed && ghostObject != null)
                        {
                            ...
                            sceneObject.scale = lastObject.transform.localScale * scale;
            ...
            {
                scale *= BaseTouchInput.KeyboardYAxis() > 0 ? 1.1f : .9f;
                if (ghostObject != null) ghostObject.transform.localScale *= Input.GetAxis("Vertical") > 0 ? 1.1f : .9f;
                if (selectedObject.tag == "Module")
                {
                    GameObject currentSelectedObj = selectedObject;
                    scaler = (currentSelectedObj.transform.localScale);
                    if (selectedObject.transform.tag != "Module" || selectedObject == null)
                    {
                        currentSelectedObj.transform.localScale = scaler;
                    }
                }
                else return;
            }
